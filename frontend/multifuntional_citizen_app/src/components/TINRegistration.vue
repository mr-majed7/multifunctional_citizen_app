  ...
  <div class="container">
    <div>
      ...
      <div v-else class="confirmation-message">
        ...
        <p>A message will be sent to your phone attaching your TIN number.</p>
      </div>
    </div>
  </div>
</template>
    ...
    async handleTINRegistration() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          alert("Failed to submit TIN application. Please try again.");
        }
      } catch (error) {
        console.error("Error submitting TIN application:", error);
        alert("Error submitting TIN application.");
      }
    },
